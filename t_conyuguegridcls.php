...
$t_conyugue_grid = NULL; // Initialize page object first

class ct_conyugue_grid extends ct_conyugue {
	...
	// Page object name
	var $PageObjName = 't_conyugue_grid';

	// Grid form hidden field names
	var $FormName = 'ft_conyuguegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_conyugue"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->CI_RUN->CurrentValue = strval($arKeys[0]); // CI_RUN
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
