...
$t_unidad_grid = NULL; // Initialize page object first

class ct_unidad_grid extends ct_unidad {
	...
	// Page object name
	var $PageObjName = 't_unidad_grid';

	// Grid form hidden field names
	var $FormName = 'ft_unidadgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_unidad"];

		}
		$this->AddUrl = "t_unidadadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Id_Unidad->CurrentValue = strval($arKeys[0]); // Id_Unidad
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_unidad_organizacional") {
			$this->Id_Unidad_Org->Visible = FALSE;
			if ($GLOBALS["t_unidad_organizacional"]->EventCancelled) $this->EventCancelled = TRUE;
		}
