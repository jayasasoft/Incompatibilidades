...
$t_mp_si_no_update = NULL; // Initialize page object first

class ct_mp_si_no_update extends ct_mp_si_no {
	...
	// Page object name
	var $PageObjName = 't_mp_si_no_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("t_mp_si_nolist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->Grado->setDbValue($this->Recordset->fields('Grado'));
					$this->grado_Si->setDbValue($this->Recordset->fields('grado_Si'));
					$this->grado_No->setDbValue($this->Recordset->fields('grado_No'));
				} else {
					if (!ew_CompareValue($this->id->DbValue, $this->Recordset->fields('id')))
						$this->id->CurrentValue = NULL;
					if (!ew_CompareValue($this->Grado->DbValue, $this->Recordset->fields('Grado')))
						$this->Grado->CurrentValue = NULL;
					if (!ew_CompareValue($this->grado_Si->DbValue, $this->Recordset->fields('grado_Si')))
						$this->grado_Si->CurrentValue = NULL;
					if (!ew_CompareValue($this->grado_No->DbValue, $this->Recordset->fields('grado_No')))
	...
	function SetupKeyValues($key) {
		...
		$sKeyFld = $key[1];
		$this->Grado->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function RenderRow() {
		...
		$this->grado_No->ViewCustomAttributes = "";
			...
			$this->Grado->EditValue = $this->Grado->CurrentValue;
			$this->Grado->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if ($this->Grado->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->grado_Si->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->grado_No->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->grado_Si->SetDbValueDef($rsnew, $this->grado_Si->CurrentValue, NULL, $this->grado_Si->ReadOnly || $this->grado_Si->MultiUpdate <> "1");

			// grado_No
			$this->grado_No->SetDbValueDef($rsnew, $this->grado_No->CurrentValue, NULL, $this->grado_No->ReadOnly || $this->grado_No->MultiUpdate <> "1");
...
// Create page object
if (!isset($t_mp_si_no_update)) $t_mp_si_no_update = new ct_mp_si_no_update();

// Page init
$t_mp_si_no_update->Page_Init();

// Page main
$t_mp_si_no_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_mp_si_no_update->Page_Render();
?>
...
var CurrentForm = ft_mp_si_noupdate = new ew_Form("ft_mp_si_noupdate", "update");

// Validate form
ft_mp_si_noupdate.Validate = function() {
...
ft_mp_si_noupdate.Form_CustomValidate = 
...
ft_mp_si_noupdate.ValidateRequired = true;
<?php } else { ?>
ft_mp_si_noupdate.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_mp_si_noupdate.Lists["x_grado_Si[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_mp_si_noupdate.Lists["x_grado_Si[]"].Options = <?php echo json_encode($t_mp_si_no->grado_Si->Options()) ?>;
ft_mp_si_noupdate.Lists["x_grado_No[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_mp_si_noupdate.Lists["x_grado_No[]"].Options = <?php echo json_encode($t_mp_si_no->grado_No->Options()) ?>;
...
<?php if (!$t_mp_si_no_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_mp_si_no_update->ShowPageHeader(); ?>
<?php
$t_mp_si_no_update->ShowMessage();
?>
<form name="ft_mp_si_noupdate" id="ft_mp_si_noupdate" class="<?php echo $t_mp_si_no_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_mp_si_no_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_mp_si_no_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_mp_si_no">
<?php if ($t_mp_si_no->CurrentAction == "F") { // Confirm page ?>
...
<?php } ?>
<?php foreach ($t_mp_si_no_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_t_mp_si_noupdate">
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"<?php echo $t_mp_si_no_update->Disabled ?>> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($t_mp_si_no->grado_Si->Visible) { // grado_Si ?>
	<div id="r_grado_Si" class="form-group">
		<label class="col-sm-2 control-label">
<?php if ($t_mp_si_no->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_grado_Si" id="u_grado_Si" value="1"<?php echo ($t_mp_si_no->grado_Si->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_mp_si_no->grado_Si->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_grado_Si" id="u_grado_Si" value="<?php echo $t_mp_si_no->grado_Si->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_mp_si_no->grado_Si->FldCaption() ?></label>
...
<input type="hidden" data-table="t_mp_si_no" data-field="x_grado_Si" name="x_grado_Si" id="x_grado_Si" value="<?php echo ew_HtmlEncode($t_mp_si_no->grado_Si->FormValue) ?>">
<?php } ?>
...
<?php if ($t_mp_si_no->grado_No->Visible) { // grado_No ?>
	<div id="r_grado_No" class="form-group">
		<label class="col-sm-2 control-label">
<?php if ($t_mp_si_no->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_grado_No" id="u_grado_No" value="1"<?php echo ($t_mp_si_no->grado_No->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_mp_si_no->grado_No->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_grado_No" id="u_grado_No" value="<?php echo $t_mp_si_no->grado_No->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_mp_si_no->grado_No->FldCaption() ?></label>
...
<input type="hidden" data-table="t_mp_si_no" data-field="x_grado_No" name="x_grado_No" id="x_grado_No" value="<?php echo ew_HtmlEncode($t_mp_si_no->grado_No->FormValue) ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_mp_si_no_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
ft_mp_si_noupdate.Init();
</script>
<?php
$t_mp_si_no_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_mp_si_no_update->Page_Terminate();
?>
