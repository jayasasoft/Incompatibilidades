...
$t_unidad_update = NULL; // Initialize page object first

class ct_unidad_update extends ct_unidad {
	...
	// Page object name
	var $PageObjName = 't_unidad_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("t_unidadlist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->Unidad->setDbValue($this->Recordset->fields('Unidad'));
				} else {
					if (!ew_CompareValue($this->Unidad->DbValue, $this->Recordset->fields('Unidad')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->Id_Unidad->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->Unidad->MultiUpdate = $objForm->GetValue("u_Unidad");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->Unidad->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->Unidad->MultiUpdate <> "" && !$this->Unidad->FldIsDetailKey && !is_null($this->Unidad->FormValue) && $this->Unidad->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Unidad
			$this->Unidad->SetDbValueDef($rsnew, $this->Unidad->CurrentValue, NULL, $this->Unidad->ReadOnly || $this->Unidad->MultiUpdate <> "1");
...
// Create page object
if (!isset($t_unidad_update)) $t_unidad_update = new ct_unidad_update();

// Page init
$t_unidad_update->Page_Init();

// Page main
$t_unidad_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_unidad_update->Page_Render();
?>
...
var CurrentForm = ft_unidadupdate = new ew_Form("ft_unidadupdate", "update");

// Validate form
ft_unidadupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_Unidad");
...
ft_unidadupdate.Form_CustomValidate = 
...
ft_unidadupdate.ValidateRequired = true;
<?php } else { ?>
ft_unidadupdate.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$t_unidad_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_unidad_update->ShowPageHeader(); ?>
<?php
$t_unidad_update->ShowMessage();
?>
<form name="ft_unidadupdate" id="ft_unidadupdate" class="<?php echo $t_unidad_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_unidad_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_unidad_update->Token ?>">
<?php } ?>
...
<?php } ?>
<?php foreach ($t_unidad_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_t_unidadupdate">
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($t_unidad->Unidad->Visible) { // Unidad ?>
	<div id="r_Unidad" class="form-group">
		<label for="x_Unidad" class="col-sm-2 control-label">
<input type="checkbox" name="u_Unidad" id="u_Unidad" value="1"<?php echo ($t_unidad->Unidad->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $t_unidad->Unidad->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_unidad->Unidad->CellAttributes() ?>>
<span id="el_t_unidad_Unidad">
<input type="text" data-table="t_unidad" data-field="x_Unidad" name="x_Unidad" id="x_Unidad" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($t_unidad->Unidad->getPlaceHolder()) ?>" value="<?php echo $t_unidad->Unidad->EditValue ?>"<?php echo $t_unidad->Unidad->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_unidad_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div>
	</div>
<?php } ?>
</div>
...
<script type="text/javascript">
ft_unidadupdate.Init();
</script>
<?php
$t_unidad_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_unidad_update->Page_Terminate();
?>
