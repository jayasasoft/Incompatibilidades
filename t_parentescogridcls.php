...
$t_parentesco_grid = NULL; // Initialize page object first

class ct_parentesco_grid extends ct_parentesco {
	...
	// Page object name
	var $PageObjName = 't_parentesco_grid';

	// Grid form hidden field names
	var $FormName = 'ft_parentescogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_parentesco"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Id_parentesco->CurrentValue = strval($arKeys[0]); // Id_parentesco
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_grados") {
			...
			if ($GLOBALS["t_grados"]->EventCancelled) $this->EventCancelled = TRUE;
		}
