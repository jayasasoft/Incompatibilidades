...
$t_mp_si_no_grid = NULL; // Initialize page object first

class ct_mp_si_no_grid extends ct_mp_si_no {
	...
	// Page object name
	var $PageObjName = 't_mp_si_no_grid';

	// Grid form hidden field names
	var $FormName = 'ft_mp_si_nogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_mp_si_no"];

		}
		$this->AddUrl = "t_mp_si_noadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 2) {
			...
			if (!is_numeric($this->Id->FormValue))
				return FALSE;
			$this->Grado->setFormValue($arrKeyFlds[1]);
		}
		return TRUE;
	}
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Grado") && $objForm->HasValue("o_Grado") && $this->Grado->CurrentValue <> $this->Grado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Grado_Si") && $objForm->HasValue("o_Grado_Si") && $this->Grado_Si->CurrentValue <> $this->Grado_Si->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Grado_No") && $objForm->HasValue("o_Grado_No") && $this->Grado_No->CurrentValue <> $this->Grado_No->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->Id->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->Grado->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->Grado_Si->setOldValue($objForm->GetValue("o_Grado_Si"));
		if (!$this->Grado_No->FldIsDetailKey) {
			$this->Grado_No->setFormValue($objForm->GetValue("x_Grado_No"));
		}
		$this->Grado_No->setOldValue($objForm->GetValue("o_Grado_No"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->Grado->CurrentValue = strval($arKeys[1]); // Grado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['Grado']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_funcionario") {
			...
			if ($GLOBALS["t_funcionario"]->EventCancelled) $this->EventCancelled = TRUE;
		}
