...
$t_parientes_mp_delete = NULL; // Initialize page object first

class ct_parientes_mp_delete extends ct_parientes_mp {
	...
	// Page object name
	var $PageObjName = 't_parientes_mp_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_parientes_mplist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_parientes_mp class, t_parientes_mpinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_parientes_mplist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_parientes_mp_delete)) $t_parientes_mp_delete = new ct_parientes_mp_delete();

// Page init
$t_parientes_mp_delete->Page_Init();

// Page main
$t_parientes_mp_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_parientes_mp_delete->Page_Render();
?>
...
var CurrentForm = ft_parientes_mpdelete = new ew_Form("ft_parientes_mpdelete", "delete");

// Form_CustomValidate event
ft_parientes_mpdelete.Form_CustomValidate = 
...
ft_parientes_mpdelete.ValidateRequired = true;
<?php } else { ?>
ft_parientes_mpdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_parientes_mpdelete.Lists["x_Grado_Parentesco"] = {"LinkField":"x_Grado","Ajax":true,"AutoFill":false,"DisplayFields":["x_Grado","x_Parentesco","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"s_parentesco_global"};
ft_parientes_mpdelete.Lists["x_Unidad_Organizacional"] = {"LinkField":"x_Unidad_Organizacional","Ajax":true,"AutoFill":false,"DisplayFields":["x_Unidad_Organizacional","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"seleccion_cargos"};
...
<?php $t_parientes_mp_delete->ShowPageHeader(); ?>
<?php
$t_parientes_mp_delete->ShowMessage();
?>
<form name="ft_parientes_mpdelete" id="ft_parientes_mpdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_parientes_mp_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_parientes_mp_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_parientes_mp">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_parientes_mp_delete->RecKeys as $key) { ?>
...
<?php if ($t_parientes_mp->Id->Visible) { // Id ?>
		<th><span id="elh_t_parientes_mp_Id" class="t_parientes_mp_Id"><?php echo $t_parientes_mp->Id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_parientes_mp->Nombres->Visible) { // Nombres ?>
		<th><span id="elh_t_parientes_mp_Nombres" class="t_parientes_mp_Nombres"><?php echo $t_parientes_mp->Nombres->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_parientes_mp->Apellido_Paterno->Visible) { // Apellido_Paterno ?>
		<th><span id="elh_t_parientes_mp_Apellido_Paterno" class="t_parientes_mp_Apellido_Paterno"><?php echo $t_parientes_mp->Apellido_Paterno->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_parientes_mp->Apellido_Materno->Visible) { // Apellido_Materno ?>
		<th><span id="elh_t_parientes_mp_Apellido_Materno" class="t_parientes_mp_Apellido_Materno"><?php echo $t_parientes_mp->Apellido_Materno->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_parientes_mp->Grado_Parentesco->Visible) { // Grado_Parentesco ?>
		<th><span id="elh_t_parientes_mp_Grado_Parentesco" class="t_parientes_mp_Grado_Parentesco"><?php echo $t_parientes_mp->Grado_Parentesco->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_parientes_mp->Parentesco->Visible) { // Parentesco ?>
		<th><span id="elh_t_parientes_mp_Parentesco" class="t_parientes_mp_Parentesco"><?php echo $t_parientes_mp->Parentesco->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_parientes_mp->Unidad_Organizacional->Visible) { // Unidad_Organizacional ?>
		<th><span id="elh_t_parientes_mp_Unidad_Organizacional" class="t_parientes_mp_Unidad_Organizacional"><?php echo $t_parientes_mp->Unidad_Organizacional->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_parientes_mp_delete->RecCnt = 0;
$i = 0;
while (!$t_parientes_mp_delete->Recordset->EOF) {
	$t_parientes_mp_delete->RecCnt++;
	$t_parientes_mp_delete->RowCnt++;

	// Set row properties
	$t_parientes_mp->ResetAttrs();
	$t_parientes_mp->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_parientes_mp_delete->LoadRowValues($t_parientes_mp_delete->Recordset);

	// Render row
	$t_parientes_mp_delete->RenderRow();
?>
	<tr<?php echo $t_parientes_mp->RowAttributes() ?>>
<?php if ($t_parientes_mp->Id->Visible) { // Id ?>
		<td<?php echo $t_parientes_mp->Id->CellAttributes() ?>>
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Id" class="t_parientes_mp_Id">
...
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Nombres" class="t_parientes_mp_Nombres">
...
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Apellido_Paterno" class="t_parientes_mp_Apellido_Paterno">
...
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Apellido_Materno" class="t_parientes_mp_Apellido_Materno">
...
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Grado_Parentesco" class="t_parientes_mp_Grado_Parentesco">
...
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Parentesco" class="t_parientes_mp_Parentesco">
...
<span id="el<?php echo $t_parientes_mp_delete->RowCnt ?>_t_parientes_mp_Unidad_Organizacional" class="t_parientes_mp_Unidad_Organizacional">
...
<?php
	$t_parientes_mp_delete->Recordset->MoveNext();
}
$t_parientes_mp_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_parientes_mp_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_parientes_mpdelete.Init();
</script>
<?php
$t_parientes_mp_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_parientes_mp_delete->Page_Terminate();
?>
