...
//
// Table class for t_parientes_mp
//
class ct_parientes_mp extends cTable {
	var $Id;
	var $Nombres;
	...
	function __construct() {
		...
		$this->TableVar = 't_parientes_mp';
		$this->TableName = 't_parientes_mp';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_parientes_mp`";
		$this->DBID = 'DB';
		...
		// Id
		$this->Id = new cField('t_parientes_mp', 't_parientes_mp', 'x_Id', 'Id', '`Id`', '`Id`', 3, -1, FALSE, '`Id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Id->Sortable = TRUE; // Allow sort
		$this->Id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Id'] = &$this->Id;

		// Nombres
		$this->Nombres = new cField('t_parientes_mp', 't_parientes_mp', 'x_Nombres', 'Nombres', '`Nombres`', '`Nombres`', 200, -1, FALSE, '`Nombres`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Nombres->Sortable = TRUE; // Allow sort
		$this->fields['Nombres'] = &$this->Nombres;

		// Apellido_Paterno
		$this->Apellido_Paterno = new cField('t_parientes_mp', 't_parientes_mp', 'x_Apellido_Paterno', 'Apellido_Paterno', '`Apellido_Paterno`', '`Apellido_Paterno`', 200, -1, FALSE, '`Apellido_Paterno`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Apellido_Paterno->Sortable = TRUE; // Allow sort
		$this->fields['Apellido_Paterno'] = &$this->Apellido_Paterno;

		// Apellido_Materno
		$this->Apellido_Materno = new cField('t_parientes_mp', 't_parientes_mp', 'x_Apellido_Materno', 'Apellido_Materno', '`Apellido_Materno`', '`Apellido_Materno`', 200, -1, FALSE, '`Apellido_Materno`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Apellido_Materno->Sortable = TRUE; // Allow sort
		$this->fields['Apellido_Materno'] = &$this->Apellido_Materno;

		// Grado_Parentesco
		$this->Grado_Parentesco = new cField('t_parientes_mp', 't_parientes_mp', 'x_Grado_Parentesco', 'Grado_Parentesco', '`Grado_Parentesco`', '`Grado_Parentesco`', 200, -1, FALSE, '`EV__Grado_Parentesco`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// Parentesco
		$this->Parentesco = new cField('t_parientes_mp', 't_parientes_mp', 'x_Parentesco', 'Parentesco', '`Parentesco`', '`Parentesco`', 200, -1, FALSE, '`Parentesco`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Parentesco->Sortable = TRUE; // Allow sort
		$this->fields['Parentesco'] = &$this->Parentesco;

		// Unidad_Organizacional
		$this->Unidad_Organizacional = new cField('t_parientes_mp', 't_parientes_mp', 'x_Unidad_Organizacional', 'Unidad_Organizacional', '`Unidad_Organizacional`', '`Unidad_Organizacional`', 200, -1, FALSE, '`Unidad_Organizacional`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_parientes_mp`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`Grado`,'" . ew_ValueSeparator(1, $this->Grado_Parentesco) . "',`Parentesco`) FROM `s_parentesco_global` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`Grado` = `t_parientes_mp`.`Grado_Parentesco` LIMIT 1) AS `EV__Grado_Parentesco` FROM `t_parientes_mp`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if ($this->BasicSearch->getKeyword() <> "")
			return TRUE;
		if ($this->Grado_Parentesco->AdvancedSearch->SearchValue <> "" ||
			$this->Grado_Parentesco->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->Grado_Parentesco->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->Grado_Parentesco->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_parientes_mpview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_parientes_mpview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_parientes_mpadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_parientes_mpadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_parientes_mpedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_parientes_mpadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_parientes_mpdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function GetAutoFill($id, $val) {
		$rsarr = array();
		$rowcnt = 0;
		if (preg_match('/^x(\d)*_Grado_Parentesco$/', $id)) {
			$conn = &$this->Connection();
			$sSqlWrk = "SELECT `Parentesco` AS FIELD0 FROM `s_parentesco_global`";
			$sWhereWrk = "(`Grado` = " . ew_QuotedValue($val, EW_DATATYPE_STRING, $this->DBID) . ")";
			$this->Grado_Parentesco->LookupFilters = array();
			$this->Lookup_Selecting($this->Grado_Parentesco, $sWhereWrk); // Call Lookup selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					$ar = array();
					$this->Parentesco->setDbValue($rs->fields[0]);
					$this->RowType == EW_ROWTYPE_EDIT;
					$this->RenderEditRow();
					$ar[] = ($this->Parentesco->AutoFillOriginalValue) ? $this->Parentesco->CurrentValue : $this->Parentesco->EditValue;
					$rowcnt += 1;
					$rsarr[] = $ar;
					$rs->MoveNext();
				}
