...
$t_re_adopcion_update = NULL; // Initialize page object first

class ct_re_adopcion_update extends ct_re_adopcion {
	...
	// Page object name
	var $PageObjName = 't_re_adopcion_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("t_re_adopcionlist.php"); // No records selected, return to list
...
// Create page object
if (!isset($t_re_adopcion_update)) $t_re_adopcion_update = new ct_re_adopcion_update();

// Page init
$t_re_adopcion_update->Page_Init();

// Page main
$t_re_adopcion_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_re_adopcion_update->Page_Render();
?>
...
var CurrentForm = ft_re_adopcionupdate = new ew_Form("ft_re_adopcionupdate", "update");

// Validate form
ft_re_adopcionupdate.Validate = function() {
...
ft_re_adopcionupdate.Form_CustomValidate = 
...
ft_re_adopcionupdate.ValidateRequired = true;
<?php } else { ?>
ft_re_adopcionupdate.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_re_adopcionupdate.Lists["x_Parentesco"] = {"LinkField":"x_Parentesco","Ajax":true,"AutoFill":false,"DisplayFields":["x_Parentesco","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"s_adopcion"};
...
<?php if (!$t_re_adopcion_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_re_adopcion_update->ShowPageHeader(); ?>
<?php
$t_re_adopcion_update->ShowMessage();
?>
<form name="ft_re_adopcionupdate" id="ft_re_adopcionupdate" class="<?php echo $t_re_adopcion_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_re_adopcion_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_re_adopcion_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_re_adopcion">
<?php if ($t_re_adopcion->CurrentAction == "F") { // Confirm page ?>
...
<?php } ?>
<?php foreach ($t_re_adopcion_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_t_re_adopcionupdate">
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"<?php echo $t_re_adopcion_update->Disabled ?>> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($t_re_adopcion->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label for="x_id" class="col-sm-2 control-label">
<?php if ($t_re_adopcion->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_id" id="u_id" value="1"<?php echo ($t_re_adopcion->id->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_re_adopcion->id->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_id" id="u_id" value="<?php echo $t_re_adopcion->id->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_re_adopcion->id->FldCaption() ?></label>
...
<?php } else { ?>
<span id="el_t_re_adopcion_id">
<input type="text" data-table="t_re_adopcion" data-field="x_id" name="x_id" id="x_id" size="30" placeholder="<?php echo ew_HtmlEncode($t_re_adopcion->id->getPlaceHolder()) ?>" value="<?php echo $t_re_adopcion->id->EditValue ?>"<?php echo $t_re_adopcion->id->EditAttributes() ?>>
</span>
<?php } ?>
<?php } else { ?>
<span id="el_t_re_adopcion_id">
...
<input type="hidden" data-table="t_re_adopcion" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($t_re_adopcion->id->FormValue) ?>">
<?php } ?>
...
<?php if ($t_re_adopcion->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_Parentesco" id="u_Parentesco" value="1"<?php echo ($t_re_adopcion->Parentesco->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_re_adopcion->Parentesco->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_Parentesco" id="u_Parentesco" value="<?php echo $t_re_adopcion->Parentesco->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_re_adopcion->Parentesco->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_re_adopcion->Parentesco->CellAttributes() ?>>
<?php if ($t_re_adopcion->CurrentAction <> "F") { ?>
<span id="el_t_re_adopcion_Parentesco">
<select data-table="t_re_adopcion" data-field="x_Parentesco" data-value-separator="<?php echo $t_re_adopcion->Parentesco->DisplayValueSeparatorAttribute() ?>" id="x_Parentesco" name="x_Parentesco"<?php echo $t_re_adopcion->Parentesco->EditAttributes() ?>>
...
<input type="hidden" data-table="t_re_adopcion" data-field="x_Parentesco" name="x_Parentesco" id="x_Parentesco" value="<?php echo ew_HtmlEncode($t_re_adopcion->Parentesco->FormValue) ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_re_adopcion_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
ft_re_adopcionupdate.Init();
</script>
<?php
$t_re_adopcion_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_re_adopcion_update->Page_Terminate();
?>
