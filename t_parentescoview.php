...
$t_parentesco_view = NULL; // Initialize page object first

class ct_parentesco_view extends ct_parentesco {
	...
	// Page object name
	var $PageObjName = 't_parentesco_view';
	...
	function __construct() {
		...
		if (@$_GET["Id_parentesco"] <> "") {
			$this->RecKey["Id_parentesco"] = $_GET["Id_parentesco"];
			$KeyUrl .= "&amp;Id_parentesco=" . urlencode($this->RecKey["Id_parentesco"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["Id_parentesco"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["Id_parentesco"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Id_parentesco"] <> "") {
				...
				$this->RecKey["Id_parentesco"] = $this->Id_parentesco->QueryStringValue;
			} elseif (@$_POST["Id_parentesco"] <> "") {
				$this->Id_parentesco->setFormValue($_POST["Id_parentesco"]);
				$this->RecKey["Id_parentesco"] = $this->Id_parentesco->FormValue;
			} else {
				$sReturnUrl = "t_parentescolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "t_parentescolist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "t_parentescolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t_parentesco\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t_parentesco',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft_parentescoview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($t_parentesco_view)) $t_parentesco_view = new ct_parentesco_view();

// Page init
$t_parentesco_view->Page_Init();

// Page main
$t_parentesco_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_parentesco_view->Page_Render();
?>
...
var CurrentForm = ft_parentescoview = new ew_Form("ft_parentescoview", "view");

// Form_CustomValidate event
ft_parentescoview.Form_CustomValidate = 
...
ft_parentescoview.ValidateRequired = true;
<?php } else { ?>
ft_parentescoview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($t_parentesco->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$t_parentesco_view->IsModal) { ?>
<?php if ($t_parentesco->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $t_parentesco_view->ExportOptions->Render("body") ?>
<?php
	foreach ($t_parentesco_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$t_parentesco_view->IsModal) { ?>
<?php if ($t_parentesco->Export == "") { ?>
...
<?php } ?>
<?php $t_parentesco_view->ShowPageHeader(); ?>
<?php
$t_parentesco_view->ShowMessage();
?>
<form name="ft_parentescoview" id="ft_parentescoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_parentesco_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_parentesco_view->Token ?>">
<?php } ?>
...
<?php if ($t_parentesco->Id_parentesco->Visible) { // Id_parentesco ?>
	<tr id="r_Id_parentesco">
		<td><span id="elh_t_parentesco_Id_parentesco"><?php echo $t_parentesco->Id_parentesco->FldCaption() ?></span></td>
		<td data-name="Id_parentesco"<?php echo $t_parentesco->Id_parentesco->CellAttributes() ?>>
<span id="el_t_parentesco_Id_parentesco" data-page="1">
<span<?php echo $t_parentesco->Id_parentesco->ViewAttributes() ?>>
<?php echo $t_parentesco->Id_parentesco->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_parentesco->id_grado->Visible) { // id_grado ?>
	<tr id="r_id_grado">
		<td><span id="elh_t_parentesco_id_grado"><?php echo $t_parentesco->id_grado->FldCaption() ?></span></td>
		<td data-name="id_grado"<?php echo $t_parentesco->id_grado->CellAttributes() ?>>
<span id="el_t_parentesco_id_grado" data-page="1">
<span<?php echo $t_parentesco->id_grado->ViewAttributes() ?>>
<?php echo $t_parentesco->id_grado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_parentesco->Parentesco->Visible) { // Parentesco ?>
	<tr id="r_Parentesco">
		<td><span id="elh_t_parentesco_Parentesco"><?php echo $t_parentesco->Parentesco->FldCaption() ?></span></td>
		<td data-name="Parentesco"<?php echo $t_parentesco->Parentesco->CellAttributes() ?>>
<span id="el_t_parentesco_Parentesco" data-page="1">
<span<?php echo $t_parentesco->Parentesco->ViewAttributes() ?>>
<?php echo $t_parentesco->Parentesco->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($t_parentesco->Export == "") { ?>
<script type="text/javascript">
ft_parentescoview.Init();
</script>
<?php } ?>
<?php
$t_parentesco_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($t_parentesco->Export == "") { ?>
...
<?php
$t_parentesco_view->Page_Terminate();
?>
