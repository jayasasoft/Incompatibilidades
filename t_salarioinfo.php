...
//
// Table class for t_salario
//
class ct_salario extends cTable {
	var $Id;
	var $Salario_Mayor_Si;
	var $Salario_Mayor_No;
	...
	function __construct() {
		...
		$this->TableVar = 't_salario';
		$this->TableName = 't_salario';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_salario`";
		$this->DBID = 'DB';
		...
		// Id
		$this->Id = new cField('t_salario', 't_salario', 'x_Id', 'Id', '`Id`', '`Id`', 3, -1, FALSE, '`Id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Id->Sortable = TRUE; // Allow sort
		$this->Id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Id'] = &$this->Id;

		// Salario_Mayor_Si
		$this->Salario_Mayor_Si = new cField('t_salario', 't_salario', 'x_Salario_Mayor_Si', 'Salario_Mayor_Si', '`Salario_Mayor_Si`', '`Salario_Mayor_Si`', 200, -1, FALSE, '`Salario_Mayor_Si`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->Salario_Mayor_Si->Sortable = TRUE; // Allow sort
		$this->Salario_Mayor_Si->OptionCount = 1;
		$this->fields['Salario_Mayor_Si'] = &$this->Salario_Mayor_Si;

		// Salario_Mayor_No
		$this->Salario_Mayor_No = new cField('t_salario', 't_salario', 'x_Salario_Mayor_No', 'Salario_Mayor_No', '`Salario_Mayor_No`', '`Salario_Mayor_No`', 200, -1, FALSE, '`Salario_Mayor_No`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->Salario_Mayor_No->Sortable = TRUE; // Allow sort
		$this->Salario_Mayor_No->OptionCount = 1;
		$this->fields['Salario_Mayor_No'] = &$this->Salario_Mayor_No;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_salario`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_salarioview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_salarioview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_salarioadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_salarioadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_salarioedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_salarioadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_salariodelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Salario_Mayor_No->Exportable) $Doc->ExportCaption($this->Salario_Mayor_No);
				} else {
					if ($this->Salario_Mayor_Si->Exportable) $Doc->ExportCaption($this->Salario_Mayor_Si);
					if ($this->Salario_Mayor_No->Exportable) $Doc->ExportCaption($this->Salario_Mayor_No);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Salario_Mayor_No->Exportable) $Doc->ExportField($this->Salario_Mayor_No);
					} else {
						if ($this->Salario_Mayor_Si->Exportable) $Doc->ExportField($this->Salario_Mayor_Si);
						if ($this->Salario_Mayor_No->Exportable) $Doc->ExportField($this->Salario_Mayor_No);
					}
