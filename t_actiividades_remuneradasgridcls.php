...
$t_actiividades_remuneradas_grid = NULL; // Initialize page object first

class ct_actiividades_remuneradas_grid extends ct_actiividades_remuneradas {
	...
	// Page object name
	var $PageObjName = 't_actiividades_remuneradas_grid';

	// Grid form hidden field names
	var $FormName = 'ft_actiividades_remuneradasgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_actiividades_remuneradas"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->Tipo_Actividad->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->Remunerada->setOldValue($objForm->GetValue("o_Remunerada"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->Tipo_Actividad->CurrentValue = strval($arKeys[1]); // Tipo_Actividad
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
