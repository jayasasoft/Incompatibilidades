...
class cchangepwd extends ct_usuario {
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 't_usuario');
	...
	function Page_Main() {
		...
		if ($bPostBack && $bValidate) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$rsold = $rs->fields;
					if (IsPasswordReset() || ew_ComparePassword($rsold['Clave'], $this->OldPassword)) {
						$bValidPwd = TRUE;
						if (!IsPasswordReset())
