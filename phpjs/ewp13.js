...
//			}
	...
	if ($.isArray(parentId) && parentId.length > 0) {
		if ($.isArray(parentId[0])) { // Array of array => data
			...
			for (var i = 0, len = parentId.length; i < len; i++) {
				var arr = parentId[i].split(" ");
				if (arr.length == 2) { // Parent field in master table
					arp[arp.length] = ew_GetOptValues(parentId[i]);
				} else if (arr.length == 1) { // Parent/Child fields in the same table
...
function ew_IsHidden(el) {
	var $ = jQuery;
	return $(el).css("display") == "none" && !$(el).closest(".dropdown-menu")[0] && !ew_IsModalLookup(el) && !ew_IsHiddenTextArea(el) || ew_GetAncestorBy(el, function(node) {
			var $node = $(node);
...
function ew_AddOptDialogShow(args) {
	...
	var success = function(data) {
		...
		for (var i = 0, len = pf.length; i < len; i++) {
			...
			ar.push(ew_GetOptValues(obj));
			ar2.push(ew_GetOptTexts(obj));
		}
...
function ew_ModalLookupShow(args) {
	...
	var list = ewForms[f.id].Lists(el.replace(/^[xy]\d*_/, "x_"));
...
function ew_ParseDate(data, format) {
	...
	if ($.isNumber(format) && format >=0 && format <= 17) {
		...
		switch (format) {
			case 0: case 2: case 8: f = def + " HH" + timesep + "mm" + timesep + "ss"; break; // EW_DATE_FORMAT + " %H:%M:%S"
