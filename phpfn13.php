...
function &DbHelper($dbid = "") {
	$dbclass = "cbd_incompatibilidades_db";
...
function CurrentProjectID() {
	...
	return "{DBEDEDF3-C0F6-4CE5-A781-F1E2EFAF0B48}";
}
...
class cExportPdf extends cExportBase {
	...
	function Export() {
		...
		$dompdf->render();
        file_put_contents($gsExportFile.'.pdf', $dompdf->output());
		if (!EW_DEBUG_ENABLED && ob_get_length())
			ob_end_clean();
...
class cBreadcrumb {
	...
	function __construct() {
		global $Language;
		$this->Links[] = array("home", "HomePage", "t_funcionariolist.php", "ewHome", "", FALSE); // Home
	}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$validateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$validateUser = $customValidateUser || ew_ComparePassword($rs->fields('Clave'), $pwd, $encrypted);
					if ($validateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('Usuario')); // Load user name
						$this->setSessionUserID($rs->fields('Codigo_usuario')); // Load User ID
						$this->setSessionParentUserID($rs->fields('Codigo_usuario')); // Load parent User ID
						if (is_null($rs->fields('Nivel_id'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('Nivel_id'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $UserTable->GetSQL($sFilter, '');
			if (($RsUser = $UserTableConn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('Codigo_usuario');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('Codigo_usuario'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`Codigo_usuario` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('Codigo_usuario'));
