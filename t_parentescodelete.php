...
$t_parentesco_delete = NULL; // Initialize page object first

class ct_parentesco_delete extends ct_parentesco {
	...
	// Page object name
	var $PageObjName = 't_parentesco_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_parentescolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_parentesco class, t_parentescoinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_parentescolist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_parentesco_delete)) $t_parentesco_delete = new ct_parentesco_delete();

// Page init
$t_parentesco_delete->Page_Init();

// Page main
$t_parentesco_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_parentesco_delete->Page_Render();
?>
...
var CurrentForm = ft_parentescodelete = new ew_Form("ft_parentescodelete", "delete");

// Form_CustomValidate event
ft_parentescodelete.Form_CustomValidate = 
...
ft_parentescodelete.ValidateRequired = true;
<?php } else { ?>
ft_parentescodelete.ValidateRequired = false; 
<?php } ?>
...
<?php $t_parentesco_delete->ShowPageHeader(); ?>
<?php
$t_parentesco_delete->ShowMessage();
?>
<form name="ft_parentescodelete" id="ft_parentescodelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_parentesco_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_parentesco_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_parentesco">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_parentesco_delete->RecKeys as $key) { ?>
...
<?php if ($t_parentesco->Parentesco->Visible) { // Parentesco ?>
		<th><span id="elh_t_parentesco_Parentesco" class="t_parentesco_Parentesco"><?php echo $t_parentesco->Parentesco->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_parentesco_delete->RecCnt = 0;
$i = 0;
while (!$t_parentesco_delete->Recordset->EOF) {
	$t_parentesco_delete->RecCnt++;
	$t_parentesco_delete->RowCnt++;

	// Set row properties
	$t_parentesco->ResetAttrs();
	$t_parentesco->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_parentesco_delete->LoadRowValues($t_parentesco_delete->Recordset);

	// Render row
	$t_parentesco_delete->RenderRow();
?>
	<tr<?php echo $t_parentesco->RowAttributes() ?>>
<?php if ($t_parentesco->Parentesco->Visible) { // Parentesco ?>
		<td<?php echo $t_parentesco->Parentesco->CellAttributes() ?>>
<span id="el<?php echo $t_parentesco_delete->RowCnt ?>_t_parentesco_Parentesco" class="t_parentesco_Parentesco">
...
<?php
	$t_parentesco_delete->Recordset->MoveNext();
}
$t_parentesco_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_parentesco_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_parentescodelete.Init();
</script>
<?php
$t_parentesco_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_parentesco_delete->Page_Terminate();
?>
