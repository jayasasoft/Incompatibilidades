...
$t_parentesco_add = NULL; // Initialize page object first

class ct_parentesco_add extends ct_parentesco {
	...
	// Page object name
	var $PageObjName = 't_parentesco_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Id_parentesco"] != "") {
				...
				$this->setKey("Id_parentesco", $this->Id_parentesco->CurrentValue); // Set up key
			} else {
				$this->setKey("Id_parentesco", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_parentescoview.php")
	...
	function LoadDefaultValues() {
		$this->id_grado->CurrentValue = NULL;
		$this->id_grado->OldValue = $this->id_grado->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->id_grado->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@id_grado@", ew_AdjustSql($this->id_grado->CurrentValue, "DB"), $sMasterFilter);
		} else {
		...
		// id_grado
		$this->id_grado->SetDbValueDef($rsnew, $this->id_grado->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($t_parentesco_add)) $t_parentesco_add = new ct_parentesco_add();

// Page init
$t_parentesco_add->Page_Init();

// Page main
$t_parentesco_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_parentesco_add->Page_Render();
?>
...
var CurrentForm = ft_parentescoadd = new ew_Form("ft_parentescoadd", "add");

// Validate form
ft_parentescoadd.Validate = function() {
...
ft_parentescoadd.Form_CustomValidate = 
...
ft_parentescoadd.ValidateRequired = true;
<?php } else { ?>
ft_parentescoadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$t_parentesco_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_parentesco_add->ShowPageHeader(); ?>
<?php
$t_parentesco_add->ShowMessage();
?>
<form name="ft_parentescoadd" id="ft_parentescoadd" class="<?php echo $t_parentesco_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_parentesco_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_parentesco_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_parentesco_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_parentescoadd.Init();
</script>
<?php
$t_parentesco_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_parentesco_add->Page_Terminate();
?>
