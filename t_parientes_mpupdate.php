...
$t_parientes_mp_update = NULL; // Initialize page object first

class ct_parientes_mp_update extends ct_parientes_mp {
	...
	// Page object name
	var $PageObjName = 't_parientes_mp_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("t_parientes_mplist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->Grado_Parentesco->setDbValue($this->Recordset->fields('Grado_Parentesco'));
					...
					if (!ew_CompareValue($this->Grado_Parentesco->DbValue, $this->Recordset->fields('Grado_Parentesco')))
	...
	function LoadFormValues() {
		...
		$this->Grado_Parentesco->MultiUpdate = $objForm->GetValue("u_Grado_Parentesco");
	...
	function ValidateForm() {
		...
		if ($this->Grado_Parentesco->MultiUpdate == "1") $lUpdateCnt++;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->Grado_Parentesco->SetDbValueDef($rsnew, $this->Grado_Parentesco->CurrentValue, "", $this->Grado_Parentesco->ReadOnly || $this->Grado_Parentesco->MultiUpdate <> "1");
...
// Create page object
if (!isset($t_parientes_mp_update)) $t_parientes_mp_update = new ct_parientes_mp_update();

// Page init
$t_parientes_mp_update->Page_Init();

// Page main
$t_parientes_mp_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_parientes_mp_update->Page_Render();
?>
...
var CurrentForm = ft_parientes_mpupdate = new ew_Form("ft_parientes_mpupdate", "update");

// Validate form
ft_parientes_mpupdate.Validate = function() {
...
ft_parientes_mpupdate.Form_CustomValidate = 
...
ft_parientes_mpupdate.ValidateRequired = true;
<?php } else { ?>
ft_parientes_mpupdate.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_parientes_mpupdate.Lists["x_Grado_Parentesco"] = {"LinkField":"x_Grado","Ajax":true,"AutoFill":true,"DisplayFields":["x_Grado","x_Parentesco","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"s_parentesco_global"};
ft_parientes_mpupdate.Lists["x_Unidad_Organizacional"] = {"LinkField":"x_Unidad_Organizacional","Ajax":true,"AutoFill":false,"DisplayFields":["x_Unidad_Organizacional","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"seleccion_cargos"};
...
<?php if (!$t_parientes_mp_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_parientes_mp_update->ShowPageHeader(); ?>
<?php
$t_parientes_mp_update->ShowMessage();
?>
<form name="ft_parientes_mpupdate" id="ft_parientes_mpupdate" class="<?php echo $t_parientes_mp_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_parientes_mp_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_parientes_mp_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_parientes_mp">
<?php if ($t_parientes_mp->CurrentAction == "F") { // Confirm page ?>
...
<?php } ?>
<?php foreach ($t_parientes_mp_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_t_parientes_mpupdate">
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"<?php echo $t_parientes_mp_update->Disabled ?>> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($t_parientes_mp->Id->Visible) { // Id ?>
	<div id="r_Id" class="form-group">
		<label for="x_Id" class="col-sm-2 control-label">
<?php if ($t_parientes_mp->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_Id" id="u_Id" value="1"<?php echo ($t_parientes_mp->Id->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_parientes_mp->Id->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_Id" id="u_Id" value="<?php echo $t_parientes_mp->Id->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_parientes_mp->Id->FldCaption() ?></label>
...
<?php } else { ?>
<span id="el_t_parientes_mp_Id">
<input type="text" data-table="t_parientes_mp" data-field="x_Id" name="x_Id" id="x_Id" size="30" placeholder="<?php echo ew_HtmlEncode($t_parientes_mp->Id->getPlaceHolder()) ?>" value="<?php echo $t_parientes_mp->Id->EditValue ?>"<?php echo $t_parientes_mp->Id->EditAttributes() ?>>
</span>
<?php } ?>
<?php } else { ?>
<span id="el_t_parientes_mp_Id">
...
<input type="hidden" data-table="t_parientes_mp" data-field="x_Id" name="x_Id" id="x_Id" value="<?php echo ew_HtmlEncode($t_parientes_mp->Id->FormValue) ?>">
<?php } ?>
...
<?php if ($t_parientes_mp->Grado_Parentesco->Visible) { // Grado_Parentesco ?>
	<div id="r_Grado_Parentesco" class="form-group">
		<label for="x_Grado_Parentesco" class="col-sm-2 control-label">
<?php if ($t_parientes_mp->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_Grado_Parentesco" id="u_Grado_Parentesco" value="1"<?php echo ($t_parientes_mp->Grado_Parentesco->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_parientes_mp->Grado_Parentesco->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_Grado_Parentesco" id="u_Grado_Parentesco" value="<?php echo $t_parientes_mp->Grado_Parentesco->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_parientes_mp->Grado_Parentesco->FldCaption() ?></label>
...
<select data-table="t_parientes_mp" data-field="x_Grado_Parentesco" data-value-separator="<?php echo $t_parientes_mp->Grado_Parentesco->DisplayValueSeparatorAttribute() ?>" id="x_Grado_Parentesco" name="x_Grado_Parentesco"<?php echo $t_parientes_mp->Grado_Parentesco->EditAttributes() ?>>
...
<input type="hidden" data-table="t_parientes_mp" data-field="x_Grado_Parentesco" name="x_Grado_Parentesco" id="x_Grado_Parentesco" value="<?php echo ew_HtmlEncode($t_parientes_mp->Grado_Parentesco->FormValue) ?>">
<?php } ?>
...
<?php if ($t_parientes_mp->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_Parentesco" id="u_Parentesco" value="1"<?php echo ($t_parientes_mp->Parentesco->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_parientes_mp->Parentesco->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_Parentesco" id="u_Parentesco" value="<?php echo $t_parientes_mp->Parentesco->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_parientes_mp->Parentesco->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_parientes_mp->Parentesco->CellAttributes() ?>>
<?php if ($t_parientes_mp->CurrentAction <> "F") { ?>
<span id="el_t_parientes_mp_Parentesco">
<input type="text" data-table="t_parientes_mp" data-field="x_Parentesco" name="x_Parentesco" id="x_Parentesco" size="30" maxlength="25" placeholder="<?php echo ew_HtmlEncode($t_parientes_mp->Parentesco->getPlaceHolder()) ?>" value="<?php echo $t_parientes_mp->Parentesco->EditValue ?>"<?php echo $t_parientes_mp->Parentesco->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_t_parientes_mp_Parentesco">
...
<input type="hidden" data-table="t_parientes_mp" data-field="x_Parentesco" name="x_Parentesco" id="x_Parentesco" value="<?php echo ew_HtmlEncode($t_parientes_mp->Parentesco->FormValue) ?>">
<?php } ?>
...
<?php if ($t_parientes_mp->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_Unidad_Organizacional" id="u_Unidad_Organizacional" value="1"<?php echo ($t_parientes_mp->Unidad_Organizacional->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($t_parientes_mp->Unidad_Organizacional->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_Unidad_Organizacional" id="u_Unidad_Organizacional" value="<?php echo $t_parientes_mp->Unidad_Organizacional->MultiUpdate ?>">
<?php } ?>
 <?php echo $t_parientes_mp->Unidad_Organizacional->FldCaption() ?></label>
...
<select data-table="t_parientes_mp" data-field="x_Unidad_Organizacional" data-value-separator="<?php echo $t_parientes_mp->Unidad_Organizacional->DisplayValueSeparatorAttribute() ?>" id="x_Unidad_Organizacional" name="x_Unidad_Organizacional"<?php echo $t_parientes_mp->Unidad_Organizacional->EditAttributes() ?>>
...
<input type="hidden" data-table="t_parientes_mp" data-field="x_Unidad_Organizacional" name="x_Unidad_Organizacional" id="x_Unidad_Organizacional" value="<?php echo ew_HtmlEncode($t_parientes_mp->Unidad_Organizacional->FormValue) ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_parientes_mp_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
ft_parientes_mpupdate.Init();
</script>
<?php
$t_parientes_mp_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_parientes_mp_update->Page_Terminate();
?>
