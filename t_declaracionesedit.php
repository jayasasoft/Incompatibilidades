...
$t_declaraciones_edit = NULL; // Initialize page object first

class ct_declaraciones_edit extends ct_declaraciones {
	...
	// Page object name
	var $PageObjName = 't_declaraciones_edit';
	...
	function Page_Main() {
		...
		if ($this->CI_RUN->CurrentValue == "") {
			$this->Page_Terminate("t_declaracioneslist.php"); // Invalid key, return to list
		}
	...
	function RenderRow() {
		...
		$this->Archivo->ViewCustomAttributes = "";
			...
			$this->Fecha->EditValue = ew_FormatDateTime($this->Fecha->EditValue, 12);
			$this->Fecha->ViewCustomAttributes = "";
...
// Create page object
if (!isset($t_declaraciones_edit)) $t_declaraciones_edit = new ct_declaraciones_edit();

// Page init
$t_declaraciones_edit->Page_Init();

// Page main
$t_declaraciones_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_declaraciones_edit->Page_Render();
?>
...
var CurrentForm = ft_declaracionesedit = new ew_Form("ft_declaracionesedit", "edit");

// Validate form
ft_declaracionesedit.Validate = function() {
...
ft_declaracionesedit.Form_CustomValidate = 
...
ft_declaracionesedit.ValidateRequired = true;
<?php } else { ?>
ft_declaracionesedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$t_declaraciones_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_declaraciones_edit->ShowPageHeader(); ?>
<?php
$t_declaraciones_edit->ShowMessage();
?>
<form name="ft_declaracionesedit" id="ft_declaracionesedit" class="<?php echo $t_declaraciones_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_declaraciones_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_declaraciones_edit->Token ?>">
<?php } ?>
...
<?php if ($t_declaraciones->CI_RUN->Visible) { // CI_RUN ?>
	<div id="r_CI_RUN" class="form-group">
		<label id="elh_t_declaraciones_CI_RUN" for="x_CI_RUN" class="col-sm-2 control-label ewLabel"><?php echo $t_declaraciones->CI_RUN->FldCaption() ?></label>
...
<p class="form-control-static"><?php echo $t_declaraciones->CI_RUN->EditValue ?></p></span>
</span>
<input type="hidden" data-table="t_declaraciones" data-field="x_CI_RUN" name="x_CI_RUN" id="x_CI_RUN" value="<?php echo ew_HtmlEncode($t_declaraciones->CI_RUN->CurrentValue) ?>">
<?php echo $t_declaraciones->CI_RUN->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t_declaraciones->Fecha->Visible) { // Fecha ?>
	<div id="r_Fecha" class="form-group">
		<label id="elh_t_declaraciones_Fecha" for="x_Fecha" class="col-sm-2 control-label ewLabel"><?php echo $t_declaraciones->Fecha->FldCaption() ?></label>
...
<p class="form-control-static"><?php echo $t_declaraciones->Fecha->EditValue ?></p></span>
</span>
<input type="hidden" data-table="t_declaraciones" data-field="x_Fecha" name="x_Fecha" id="x_Fecha" value="<?php echo ew_HtmlEncode($t_declaraciones->Fecha->CurrentValue) ?>">
...
<?php if ((!ew_EmptyStr($t_declaraciones->Archivo->EditValue)) && $t_declaraciones->Archivo->LinkAttributes() <> "") { ?>
<a<?php echo $t_declaraciones->Archivo->LinkAttributes() ?>><p class="form-control-static"><?php echo $t_declaraciones->Archivo->EditValue ?></p></a>
<?php } else { ?>
<p class="form-control-static"><?php echo $t_declaraciones->Archivo->EditValue ?></p>
<?php } ?>
</span>
</span>
<input type="hidden" data-table="t_declaraciones" data-field="x_Archivo" name="x_Archivo" id="x_Archivo" value="<?php echo ew_HtmlEncode($t_declaraciones->Archivo->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_declaraciones_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_declaracionesedit.Init();
</script>
<?php
$t_declaraciones_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_declaraciones_edit->Page_Terminate();
?>
