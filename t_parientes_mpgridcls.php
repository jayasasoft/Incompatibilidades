...
$t_parientes_mp_grid = NULL; // Initialize page object first

class ct_parientes_mp_grid extends ct_parientes_mp {
	...
	// Page object name
	var $PageObjName = 't_parientes_mp_grid';

	// Grid form hidden field names
	var $FormName = 'ft_parientes_mpgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_parientes_mp"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('Nombres');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('Apellido_Paterno');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('Apellido_Materno');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 3) {
			if (strval($arKeys[0]) <> "")
				$this->Nombres->CurrentValue = strval($arKeys[0]); // Nombres
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->Apellido_Paterno->CurrentValue = strval($arKeys[1]); // Apellido_Paterno
			else
				$bValidKey = FALSE;
			if (strval($arKeys[2]) <> "")
				$this->Apellido_Materno->CurrentValue = strval($arKeys[2]); // Apellido_Materno
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
